...
function SupportingPanel({ sections }: { sections: CanonicalSection[] }) {
  return (
    <GlassCard style={{ flex: 1, minHeight: 0, gap: 12, padding: '18px 20px', overflow: 'hidden' }}>
      {sections.map((section, index) => (
        <div key={section.id} style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, gap: 9, paddingTop: index === 0 ? 0 : 10, borderTop: index === 0 ? 'none' : `1px solid ${TOKENS.colors.border}` }}>
          <SectionTitle title={section.title} description={section.description} compact />
...
export function QaLayout({ data, profile, width = 1600, height = 1120 }: QaLayoutProps) {
  ...
  const leftLower = process ?? supporting[0];
  const rightSupporting = supporting.filter((section) => section !== leftLower);
  const passed = checklist?.items.filter((item) => item.status === 'passed').length ?? 0;
  const total = checklist?.items.length ?? 0;
  const padding = Math.round(TOKENS.spacing.xl * profile.paddingScale);
    ...
    <PosterShell width={width} height={height} padding={padding}>
      <div style={{ display: 'flex', flex: 1, minHeight: 0, gap: TOKENS.spacing.md }}>
        <div style={{ display: 'flex', flexDirection: 'column', width: '51%', minWidth: 0, gap: TOKENS.spacing.md }}>
          <GlassCard accent style={{ flex: 1.15, minHeight: 0, justifyContent: 'space-between', padding: '34px 36px', overflow: 'hidden' }}>
          ...
          {leftLower ? <SectionCard section={leftLower} compact style={{ flex: 0.85, minHeight: 0, padding: '19px 22px', overflow: 'hidden' }} /> : (
            <GlassCard style={{ flex: 0.85, alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', width: '49%', minWidth: 0, gap: TOKENS.spacing.md }}>
          {checklist ? (
            <GlassCard style={{ flex: 1.1, minHeight: 0, gap: 10, padding: '18px 20px', overflow: 'hidden' }}>
          ...
          {metrics.length > 0 ? (
            <GlassCard style={{ flex: 0.8, minHeight: 0, gap: 10, padding: '17px 20px', overflow: 'hidden' }}>
